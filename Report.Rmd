...
***
## Regression Results
In the regression analysis, we modeled membership in the Top 5% expenditure group as a binary outcome and examined how demographics, socioeconomic status, and clinical burden are associated with high-cost status. In the baseline survey-weighted logistic model including age group, sex, race/ethnicity, poverty category, insurance coverage, and the number of distinct CCSR condition categories, age and multi-morbidity were the most important predictors. Compared with children and younger adults, individuals aged 45–64 and 65+ had substantially higher odds of being high-cost, and each additional CCSR category was associated with a sizeable increase in the odds of belonging to the Top 5% group. Associations for race/ethnicity and poverty categories were generally small and imprecise after adjustment, and the estimated effect of sex attenuated once clinical covariates were included.

<table style="width:100%; border-collapse:collapse;">
<tr>
  <td style="width:50%; text-align:center; padding-right:10px;">
    <img src="img/top20.png" style="width:95%;" />
  </td>
  <td style="width:50%; text-align:center; padding-left:10px;">
    <img src="img/top20_1.png" style="width:95%;" />
  </td>
</tr>
</table>

We then augmented the baseline model with binary indicators for the most prevalent CCSR categories and constructed a reduced “selected” model retaining only statistically significant diagnosis indicators. Both the full and selected models achieved lower survey-weighted AIC than the baseline model, indicating improved in-sample fit. However, repeated K-fold cross-validation revealed very similar RMSE distributions for the baseline, full, and selected models, with substantial overlap across the three specifications. This pattern suggests that while adding diagnosis-specific indicators improves model fit to the observed data, it yields only modest gains in out-of-sample predictive accuracy. Overall, the count of distinct CCSR categories—the simple multi-morbidity measure—remains the dominant and most stable predictor of high-cost status, whereas individual diagnoses contribute relatively little incremental predictive information once multi-morbidity and age are taken into account.

![](img/reg.png){width=70%}
...
## ------------------------------------------------------------------
## 1. Construct multimorbidity measures and disease indicators
## ------------------------------------------------------------------
...
# counts of conditions by CCSR code
...
# compare top 10 CCSR codes (unweighted vs weighted)
data.frame(
...
# create binary indicator for each CCSR in ccsr_union
...
## ------------------------------------------------------------------
## 2. Merge person-level data and build survey design for modeling
## ------------------------------------------------------------------

fyc23_model <- fyc23_clean %>%
  left_join(person_ccsr, by = "dupersid") %>% 
  mutate(
    high_cost = ifelse(exp_group == "Top 5%", 1, 0)  # Top 5% vs others
  )
...
## ------------------------------------------------------------------
## 3. Logistic regression models and feature selection
## ------------------------------------------------------------------
...
# full model with specific high-prevalence CCSR indicators
ccsr_vars <- c(
...
# reduced model with selected significant CCSR indicators
selected_vars <- c(
  "age_cat4",
  "sex_f",
  "povcat_f",
...
# AIC comparison (survey-weighted effective AIC)
AIC(model_baseline)
AIC(model_full)
AIC(model_selected)

## ------------------------------------------------------------------
## 4. Cross-validated prediction error (RMSE)
## ------------------------------------------------------------------

# K-fold CV function for RMSE
...
# visualize RMSE distributions for the three models
...
***
## Discussion
Our findings show that extremely high health care spending in 2023 is concentrated among a small subset of older, multiply-morbid individuals, and that a parsimonious set of variables—age, insurance coverage, and a simple measure of multi-morbidity—captures most of the predictive signal available in MEPS. More complex models that incorporate detailed diagnosis indicators improve statistical fit but do not substantially enhance the ability to distinguish Top-5% spenders from the rest of the population. This underscores both the central role of multi-morbid chronic disease in driving expenditures and the limitations of cross-sectional survey data for prospective “hot-spotting.” From a policy perspective, these results suggest that risk-stratification and care-management efforts may benefit more from simple, robust measures of condition burden and age, coupled with system-level interventions to coordinate chronic care, rather than from increasingly complex predictive models built on limited cross-sectional information.
